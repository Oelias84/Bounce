...
open class AKPickerView: UIView, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, AKCollectionViewLayoutDelegate {
    ...
    @IBInspectable var maskDisabled: Bool = false {
        didSet {
...
//        self.maskDisabled = self.maskDisabled == false : self.maskDisabled
    }
    ...
    fileprivate func didEndScrolling() {
        ...
        case .flat:
            ...
            if let indexPath = self.collectionView.indexPathForItem(at: center) {
                self.selectItem((indexPath as NSIndexPath).item, animated: true, notifySelection: true)
            }
        case .wheel:
    ...
    open func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let title = self.dataSource?.pickerView?(self, titleForItem: (indexPath as NSIndexPath).item) {
            ...
            if let delegate = self.delegate {
                delegate.pickerView?(self, configureLabel: cell.label, forItem: (indexPath as NSIndexPath).item)
                if let margin = delegate.pickerView?(self, marginForItem: (indexPath as NSIndexPath).item) {
                    cell.label.frame = cell.label.frame.insetBy(dx: -margin.width, dy: -margin.height)
                }
            }
        } else if let image = self.dataSource?.pickerView?(self, imageForItem: (indexPath as NSIndexPath).item) {
            // briefOS
            let currentSize = image.size
            if ((indexPath as NSIndexPath).item == self.selectedItem) {
                cell.imageView.image = image.imageWithSize(currentSize)
                cell.imageView.alpha = 1.0
            } else {
                let reducedSize = CGSize(width: currentSize.width * 0.7, height: currentSize.height * 0.7)
                cell.imageView.image = image.imageWithSize(reducedSize)
                cell.imageView.alpha = 0.65
            }
            // briefOS
        }
        cell._selected = ((indexPath as NSIndexPath).item == self.selectedItem)
        return cell
    }
    ...
    open func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        if let title = self.dataSource?.pickerView?(self, titleForItem: (indexPath as NSIndexPath).item) {
            ...
            if let margin = self.delegate?.pickerView?(self, marginForItem: (indexPath as NSIndexPath).item) {
    ...
    open func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        self.selectItem((indexPath as NSIndexPath).item, animated: true)
    }
