...
extension MessagesManager {
	...
	public func postMassage(isNewChat: Bool, existingChatId: String?, otherUserEmail: String? ,messageText: String, chatOtherTokens: [String]?) {
	...
	public func supportChatExist(completion: @escaping (Chat?) -> ()) {
		...
		GoogleDatabaseManager.shared.getAllChats(for: userSafeEmail) {
			...
			case .success(let chats):
				...
				completion(chats.first(where: { $0.otherUserEmail == "support-mail-com" }))
			case .failure(let error):
				print(error.localizedDescription)
				completion(nil)
			}
